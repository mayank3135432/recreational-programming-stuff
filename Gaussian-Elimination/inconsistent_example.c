...
int main(){
  ...
  matrix[2][0]=0;matrix[2][1]=0;matrix[2][2]=0;matrix[2][3]=1;
  display(matrix,3,4);
  double* v=solve(matrix,3,4);
  if(v==NULL){
    printf("Inconsistent or Dependent System of Equations\n");
    return 0; // stop the program. Else NULLPOINTERDEREFERENCE will happen
  }
  display(matrix,3,4);
  for(int i=0;i<3;i++){
    printf("%lf,",v[i]);

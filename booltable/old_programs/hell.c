...
bool evaluate(int value, int num){
    //int* evalstack=(int*)malloc(MAX*sizeof(int));
    /* bool* values = malloc(num*sizeof(bool));
    ...
    for(i=0;i<=load;i++){
       ...
       if(isletter(postfix[i])){
        for(j=0;j<num;j++){
            if(postfix[i]==varnames[j]) evalstack[++evaltop]=(num&(1<<j))>>j;
        }
       }
...
int main(){
    ...
    printf("\n%d\n",numvar);
     ...
     // evaluate
    int NUV=pov2(numvar);
    for(int i=NUV-1;i>=0;i--){
        for(int j=0;j<numvar;j++){
            printf("%d|",(i&(1<<j))>>j);
        }
        printf("\b || %d",evaluate(i,numvar));
        printf("\n");
    }
